...
class Solipsis5(IStrategy):
    ...
    # Base Pair Params
    base_mp = IntParameter(10, 50, default=30, space='buy', optimize=True)
    base_rmi_max = IntParameter(30, 60, default=50, space='buy', optimize=True)
    base_rmi_min = IntParameter(0, 30, default=20, space='buy', optimize=True)
    base_ma_streak = IntParameter(1, 4, default=1, space='buy', optimize=True)
    base_rmi_streak = IntParameter(3, 8, default=3, space='buy', optimize=True)
    base_trigger = CategoricalParameter(['pcc', 'rmi', 'none'], default='rmi', space='buy', optimize=True)
    inf_pct_adr = DecimalParameter(0.70, 0.99, default=0.80, space='buy', optimize=True)
    # BTC Informative
    xbtc_guard = CategoricalParameter(['strict', 'lazy', 'none'], default='lazy', space='buy', optimize=True)
    xbtc_base_rmi = IntParameter(20, 70, default=40, space='buy', optimize=True)
    # BTC / ETH Stake Parameters
    xtra_base_stake_rmi = IntParameter(10, 50, default=50, space='buy', optimize=True)
    xtra_base_fiat_rmi = IntParameter(30, 70, default=50, space='buy', optimize=True)
    ...
    csell_roi_time = IntParameter(720, 1440, default=720, space='sell', optimize=True)
    csell_roi_start = DecimalParameter(0.01, 0.05, default=0.01, space='sell', optimize=True)
    csell_roi_end = DecimalParameter(0.0, 0.01, default=0, space='sell', optimize=True)
    csell_trend_type = CategoricalParameter(['rmi', 'ssl', 'candle', 'any', 'none'], default='any', space='sell', optimize=True)
    csell_pullback = CategoricalParameter([True, False], default=True, space='sell', optimize=True)
    csell_pullback_amount = DecimalParameter(0.005, 0.03, default=0.01, space='sell', optimize=True)
    csell_pullback_respect_roi = CategoricalParameter([True, False], default=False, space='sell', optimize=True)
    csell_endtrend_respect_roi = CategoricalParameter([True, False], default=False, space='sell', optimize=True)

    # Custom Stoploss
    cstop_loss_threshold = DecimalParameter(-0.05, -0.01, default=-0.03, space='sell', optimize=True)
    cstop_bail_how = CategoricalParameter(['roc', 'time', 'any', 'none'], default='none', space='sell', optimize=True)
    cstop_bail_roc = DecimalParameter(-5.0, -1.0, default=-3.0, space='sell', optimize=True)
    cstop_bail_time = IntParameter(60, 1440, default=720, space='sell', optimize=True)
    cstop_bail_time_trend = CategoricalParameter([True, False], default=True, space='sell', optimize=True)
