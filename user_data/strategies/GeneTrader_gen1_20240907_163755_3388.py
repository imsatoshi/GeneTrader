...
class GeneTrader_gen1_20240907_163755_3388(IStrategy):
    INTERFACE_VERSION = 3
    ...
    csl = {}
    commands = []
    initial_entry_ratio = DecimalParameter(0.4, 1.0, default=0.77, space='buy', optimize=True)

    new_sl_coef = DecimalParameter(0.3, 0.9, default=0.47, space='sell', optimize=False)

    # TTF
    lookback_length = IntParameter(1, 30, default=30, space='buy', optimize=True)
    upper_trigger_level = IntParameter(1, 300, default=93, space='buy', optimize=True)
    lower_trigger_level = IntParameter(-300, -1, default=-150, space='buy', optimize=True)

    # Hyperoptable parameters
    buy_rsi = IntParameter(25, 60, default=35, space='buy', optimize=False)
    sell_rsi = IntParameter(50, 70, default=50, space='sell', optimize=False)

    # ATR based stop loss parameters
    atr_multiplier = DecimalParameter(1.0, 3.0, default=1.3, space='stoploss', optimize=False)

    # SWINGS
    swing_window = IntParameter(10, 50, default=23, space='buy', optimize=False)
    swing_min_periods = IntParameter(1, 10, default=3, space='buy', optimize=False)
    swing_buffer = DecimalParameter(0.01, 0.1, default=0.05, space='buy', optimize=False)

    buy_macd = DecimalParameter(-0.02, 0.02, default=0.02, space='buy', optimize=False)
    buy_ema_short = IntParameter(5, 50, default=23, space='buy', optimize=False)
    buy_ema_long = IntParameter(50, 200, default=91, space='buy', optimize=False)
    ...
    sell_ema_long = IntParameter(50, 200, default=199, space='sell', optimize=False)

    volume_dca_int = IntParameter(1, 30, default=25, space='buy', optimize=False)
    a_vol_coef = DecimalParameter(1, 2, default=1.5, space='buy', optimize=False)
    dca_candles_modulo = IntParameter(1, 100, default=60, space='buy', optimize=True)
    dca_threshold = DecimalParameter(0.01, 0.5, default=0.12, space='buy', optimize=False)

    dca_multiplier = DecimalParameter(1.0, 2.0, default=2.0, space='buy', optimize=True)
    max_dca_orders = IntParameter(1, 5, default=4, space='buy', optimize=True)
    dca_profit_threshold = DecimalParameter(-0.20, -0.05, default=-0.14, space='buy', optimize=True)
