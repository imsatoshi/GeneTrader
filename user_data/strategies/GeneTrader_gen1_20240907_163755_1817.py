...
class GeneTrader_gen1_20240907_163755_1817(IStrategy):
    INTERFACE_VERSION = 3
    ...
    new_sl_coef = DecimalParameter(0.3, 0.9, default=0.7, space='sell', optimize=False)

    # TTF
    lookback_length = IntParameter(1, 30, default=12, space='buy', optimize=True)
    upper_trigger_level = IntParameter(1, 300, default=62, space='buy', optimize=True)
    lower_trigger_level = IntParameter(-300, -1, default=-104, space='buy', optimize=True)

    # Hyperoptable parameters
    buy_rsi = IntParameter(25, 60, default=38, space='buy', optimize=False)
    sell_rsi = IntParameter(50, 70, default=64, space='sell', optimize=False)

    # ATR based stop loss parameters
    atr_multiplier = DecimalParameter(1.0, 3.0, default=1.2, space='stoploss', optimize=False)

    # SWINGS
    swing_window = IntParameter(10, 50, default=46, space='buy', optimize=False)
    swing_min_periods = IntParameter(1, 10, default=3, space='buy', optimize=False)
    swing_buffer = DecimalParameter(0.01, 0.1, default=0.04, space='buy', optimize=False)

    buy_macd = DecimalParameter(-0.02, 0.02, default=-0.01, space='buy', optimize=False)
    buy_ema_short = IntParameter(5, 50, default=30, space='buy', optimize=False)
    buy_ema_long = IntParameter(50, 200, default=109, space='buy', optimize=False)

    sell_macd = DecimalParameter(-0.02, 0.02, default=0.0, space='sell', optimize=False)
    sell_ema_short = IntParameter(5, 50, default=45, space='sell', optimize=False)
    sell_ema_long = IntParameter(50, 200, default=93, space='sell', optimize=False)

    volume_dca_int = IntParameter(1, 30, default=22, space='buy', optimize=False)
    a_vol_coef = DecimalParameter(1, 2, default=1.8, space='buy', optimize=False)
    dca_candles_modulo = IntParameter(1, 100, default=44, space='buy', optimize=True)
    dca_threshold = DecimalParameter(0.01, 0.5, default=0.29, space='buy', optimize=False)

    dca_multiplier = DecimalParameter(1.0, 2.0, default=1.2, space='buy', optimize=True)
    max_dca_orders = IntParameter(1, 5, default=3, space='buy', optimize=True)
    dca_profit_threshold = DecimalParameter(-0.20, -0.05, default=-0.15, space='buy', optimize=True)
