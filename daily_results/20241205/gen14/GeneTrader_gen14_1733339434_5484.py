...
class GeneTrader_gen14_1733339434_5484(IStrategy):
    INTERFACE_VERSION = 2
    ...
    pullback_window = IntParameter(10.0, 50.0, default=48, space='buy', optimize=True)
    pullback_threshold = DecimalParameter(1.5, 3.0, default=2.5, space='buy', optimize=True)
    use_pullback_protection = CategoricalParameter([True, False], default=True, space='buy', optimize=True)
    ...
    # Multi Offset
    base_nb_candles_buy = IntParameter(5.0, 80.0, default=13, space='buy', optimize=True)
    base_nb_candles_sell = IntParameter(5.0, 80.0, default=78, space='sell', optimize=True)
    low_offset_sma = DecimalParameter(0.9, 0.99, default=0.97, space='buy', optimize=True)
    high_offset_sma = DecimalParameter(0.99, 1.1, default=1.01, space='sell', optimize=True)
    low_offset_ema = DecimalParameter(0.9, 0.99, default=0.92, space='buy', optimize=True)
    high_offset_ema = DecimalParameter(0.99, 1.1, default=1.08, space='sell', optimize=True)
    low_offset_trima = DecimalParameter(0.9, 0.99, default=0.95, space='buy', optimize=True)
    high_offset_trima = DecimalParameter(0.99, 1.1, default=1.08, space='sell', optimize=True)
    ...
    # Protection
    ewo_low = DecimalParameter(-20.0, -8.0, default=-14.1, space='buy', optimize=True)
    ewo_high = DecimalParameter(2.0, 12.0, default=11.3, space='buy', optimize=True)
    fast_ewo = IntParameter(10.0, 50.0, default=44, space='buy', optimize=True)
    slow_ewo = IntParameter(100.0, 200.0, default=177, space='buy', optimize=True)
    ...
    roi_dynamic_enable = CategoricalParameter([True, False], default=True, space='sell', optimize=True)
    roi_atr_multiplier = DecimalParameter(1.0, 3.0, default=2.3, space='sell', optimize=True)
    roi_adx_threshold = DecimalParameter(20.0, 40.0, default=38.5, space='sell', optimize=True)
    roi_time_weights = DecimalParameter(0.5, 1.0, default=0.9, space='sell', optimize=True)
    ...
    buy_condition_14_enable = CategoricalParameter([True, False], default=True, space='buy', optimize=True)
    ...
    buy_condition_22_enable = CategoricalParameter([True, False], default=True, space='buy', optimize=True)

    # Normal dips
    buy_dip_threshold_1 = DecimalParameter(0.001, 0.05, default=0.005, space='buy', optimize=True)
    buy_dip_threshold_2 = DecimalParameter(0.01, 0.2, default=0.018, space='buy', optimize=True)
    buy_dip_threshold_3 = DecimalParameter(0.05, 0.4, default=0.375, space='buy', optimize=True)
    buy_dip_threshold_4 = DecimalParameter(0.2, 0.5, default=0.48, space='buy', optimize=True)
    # Strict dips
    buy_dip_threshold_5 = DecimalParameter(0.001, 0.05, default=0.027, space='buy', optimize=True)
    buy_dip_threshold_6 = DecimalParameter(0.01, 0.2, default=0.019, space='buy', optimize=True)
    buy_dip_threshold_7 = DecimalParameter(0.05, 0.4, default=0.094, space='buy', optimize=True)
    buy_dip_threshold_8 = DecimalParameter(0.2, 0.5, default=0.222, space='buy', optimize=True)
    # Loose dips
    buy_dip_threshold_9 = DecimalParameter(0.001, 0.05, default=0.017, space='buy', optimize=True)
    buy_dip_threshold_10 = DecimalParameter(0.01, 0.2, default=0.05, space='buy', optimize=True)
    buy_dip_threshold_11 = DecimalParameter(0.05, 0.4, default=0.116, space='buy', optimize=True)
    buy_dip_threshold_12 = DecimalParameter(0.2, 0.5, default=0.215, space='buy', optimize=True)

    # 24 hours
    buy_pump_pull_threshold_1 = DecimalParameter(1.5, 3.0, default=1.83, space='buy', optimize=True)
    buy_pump_threshold_1 = DecimalParameter(0.4, 1.0, default=0.427, space='buy', optimize=True)
    # 36 hours
    buy_pump_pull_threshold_2 = DecimalParameter(1.5, 3.0, default=1.5, space='buy', optimize=True)
    buy_pump_threshold_2 = DecimalParameter(0.4, 1.0, default=0.592, space='buy', optimize=True)
    # 48 hours
    buy_pump_pull_threshold_3 = DecimalParameter(1.5, 3.0, default=2.88, space='buy', optimize=True)
    buy_pump_threshold_3 = DecimalParameter(0.4, 1.0, default=0.89, space='buy', optimize=True)

    # 24 hours strict
    buy_pump_pull_threshold_4 = DecimalParameter(1.5, 3.0, default=2.6, space='buy', optimize=True)
    buy_pump_threshold_4 = DecimalParameter(0.4, 1.0, default=0.955, space='buy', optimize=True)
    # 36 hours strict
    buy_pump_pull_threshold_5 = DecimalParameter(1.5, 3.0, default=2.75, space='buy', optimize=True)
    buy_pump_threshold_5 = DecimalParameter(0.4, 1.0, default=0.973, space='buy', optimize=True)
    # 48 hours strict
    buy_pump_pull_threshold_6 = DecimalParameter(1.5, 3.0, default=1.83, space='buy', optimize=True)
    buy_pump_threshold_6 = DecimalParameter(0.4, 1.0, default=0.782, space='buy', optimize=True)

    # 24 hours loose
    buy_pump_pull_threshold_7 = DecimalParameter(1.5, 3.0, default=1.71, space='buy', optimize=True)
    buy_pump_threshold_7 = DecimalParameter(0.4, 1.0, default=0.508, space='buy', optimize=True)
    # 36 hours loose
    buy_pump_pull_threshold_8 = DecimalParameter(1.5, 3.0, default=2.94, space='buy', optimize=True)
    buy_pump_threshold_8 = DecimalParameter(0.4, 1.0, default=0.501, space='buy', optimize=True)
    # 48 hours loose
    buy_pump_pull_threshold_9 = DecimalParameter(1.5, 3.0, default=2.15, space='buy', optimize=True)
    buy_pump_threshold_9 = DecimalParameter(0.4, 1.8, default=0.953, space='buy', optimize=True)

    buy_min_inc_1 = DecimalParameter(0.01, 0.05, default=0.014, space='buy', optimize=True)
    buy_rsi_1h_min_1 = DecimalParameter(25.0, 40.0, default=30.0, space='buy', optimize=True)
    buy_rsi_1h_max_1 = DecimalParameter(70.0, 90.0, default=82.2, space='buy', optimize=True)
    buy_rsi_1 = DecimalParameter(20.0, 40.0, default=28.7, space='buy', optimize=True)
    buy_mfi_1 = DecimalParameter(20.0, 40.0, default=21.9, space='buy', optimize=True)

    buy_volume_2 = DecimalParameter(1.0, 10.0, default=5.6, space='buy', optimize=True)
    buy_rsi_1h_min_2 = DecimalParameter(30.0, 40.0, default=32.9, space='buy', optimize=True)
    buy_rsi_1h_max_2 = DecimalParameter(70.0, 95.0, default=83.7, space='buy', optimize=True)
    buy_rsi_1h_diff_2 = DecimalParameter(30.0, 50.0, default=48.7, space='buy', optimize=True)
    buy_mfi_2 = DecimalParameter(30.0, 56.0, default=37.7, space='buy', optimize=True)
    ...
    buy_ema_rel_3 = DecimalParameter(0.97, 0.999, default=0.972, space='buy', optimize=True)

    buy_bb20_close_bblowerband_4 = DecimalParameter(0.96, 0.99, default=1.0, space='buy', optimize=True)
    buy_bb20_volume_4 = DecimalParameter(1.0, 20.0, default=2.54, space='buy', optimize=True)

    buy_ema_open_mult_5 = DecimalParameter(0.016, 0.03, default=0.024, space='buy', optimize=True)
    buy_bb_offset_5 = DecimalParameter(0.98, 1.0, default=0.983, space='buy', optimize=True)
    buy_ema_rel_5 = DecimalParameter(0.97, 0.999, default=0.975, space='buy', optimize=True)

    buy_ema_open_mult_6 = DecimalParameter(0.02, 0.03, default=0.024, space='buy', optimize=True)
    buy_bb_offset_6 = DecimalParameter(0.98, 0.999, default=0.993, space='buy', optimize=True)

    buy_volume_7 = DecimalParameter(1.0, 10.0, default=8.6, space='buy', optimize=True)
    buy_ema_open_mult_7 = DecimalParameter(0.02, 0.04, default=0.024, space='buy', optimize=True)
    buy_rsi_7 = DecimalParameter(24.0, 50.0, default=46.8, space='buy', optimize=True)
    buy_ema_rel_7 = DecimalParameter(0.97, 0.999, default=0.986, space='buy', optimize=True)

    buy_volume_8 = DecimalParameter(1.0, 6.0, default=4.9, space='buy', optimize=True)
    buy_rsi_8 = DecimalParameter(36.0, 40.0, default=37.6, space='buy', optimize=True)
    buy_tail_diff_8 = DecimalParameter(3.0, 10.0, default=5.6, space='buy', optimize=True)

    buy_volume_9 = DecimalParameter(1.0, 4.0, default=1.09, space='buy', optimize=True)
    buy_ma_offset_9 = DecimalParameter(0.94, 0.99, default=0.988, space='buy', optimize=True)
    buy_bb_offset_9 = DecimalParameter(0.97, 0.99, default=0.987, space='buy', optimize=True)
    buy_rsi_1h_min_9 = DecimalParameter(26.0, 40.0, default=39.1, space='buy', optimize=True)
    buy_rsi_1h_max_9 = DecimalParameter(70.0, 90.0, default=75.0, space='buy', optimize=True)
    buy_mfi_9 = DecimalParameter(36.0, 65.0, default=64.3, space='buy', optimize=True)

    buy_volume_10 = DecimalParameter(1.0, 8.0, default=4.4, space='buy', optimize=True)
    buy_ma_offset_10 = DecimalParameter(0.93, 0.97, default=0.963, space='buy', optimize=True)
    buy_bb_offset_10 = DecimalParameter(0.97, 0.99, default=0.988, space='buy', optimize=True)
    buy_rsi_1h_10 = DecimalParameter(20.0, 40.0, default=32.7, space='buy', optimize=True)

    buy_ma_offset_11 = DecimalParameter(0.93, 0.99, default=0.972, space='buy', optimize=True)
    buy_min_inc_11 = DecimalParameter(0.005, 0.05, default=0.007, space='buy', optimize=True)
    buy_rsi_1h_min_11 = DecimalParameter(40.0, 60.0, default=56.1, space='buy', optimize=True)
    buy_rsi_1h_max_11 = DecimalParameter(70.0, 90.0, default=84.9, space='buy', optimize=True)
    buy_rsi_11 = DecimalParameter(30.0, 48.0, default=42.8, space='buy', optimize=True)
    buy_mfi_11 = DecimalParameter(36.0, 56.0, default=51.2, space='buy', optimize=True)

    buy_volume_12 = DecimalParameter(1.0, 10.0, default=1.2, space='buy', optimize=True)
    buy_ma_offset_12 = DecimalParameter(0.93, 0.97, default=0.949, space='buy', optimize=True)
    buy_rsi_12 = DecimalParameter(26.0, 40.0, default=28.1, space='buy', optimize=True)
    buy_ewo_12 = DecimalParameter(2.0, 6.0, default=3.1, space='buy', optimize=True)

    buy_volume_13 = DecimalParameter(1.0, 10.0, default=6.4, space='buy', optimize=True)
    buy_ma_offset_13 = DecimalParameter(0.93, 0.98, default=0.955, space='buy', optimize=True)
    buy_ewo_13 = DecimalParameter(-14.0, -7.0, default=-13.5, space='buy', optimize=True)

    buy_volume_14 = DecimalParameter(1.0, 10.0, default=1.8, space='buy', optimize=True)
    buy_ema_open_mult_14 = DecimalParameter(0.01, 0.03, default=0.013, space='buy', optimize=True)
    buy_bb_offset_14 = DecimalParameter(0.98, 1.0, default=0.995, space='buy', optimize=True)
    buy_ma_offset_14 = DecimalParameter(0.93, 0.99, default=0.939, space='buy', optimize=True)

    buy_volume_15 = DecimalParameter(1.0, 10.0, default=8.9, space='buy', optimize=True)
    buy_ema_open_mult_15 = DecimalParameter(0.02, 0.04, default=0.024, space='buy', optimize=True)
    buy_ma_offset_15 = DecimalParameter(0.93, 0.99, default=0.945, space='buy', optimize=True)
    buy_rsi_15 = DecimalParameter(30.0, 50.0, default=40.9, space='buy', optimize=True)
    buy_ema_rel_15 = DecimalParameter(0.97, 0.999, default=0.989, space='buy', optimize=True)

    buy_volume_16 = DecimalParameter(1.0, 10.0, default=2.3, space='buy', optimize=True)
    buy_ma_offset_16 = DecimalParameter(0.93, 0.97, default=0.939, space='buy', optimize=True)
    buy_rsi_16 = DecimalParameter(26.0, 50.0, default=41.4, space='buy', optimize=True)
    buy_ewo_16 = DecimalParameter(4.0, 8.0, default=5.7, space='buy', optimize=True)

    buy_volume_17 = DecimalParameter(0.5, 8.0, default=3.3, space='buy', optimize=True)
    buy_ma_offset_17 = DecimalParameter(0.93, 0.98, default=0.97, space='buy', optimize=True)
    buy_ewo_17 = DecimalParameter(-18.0, -10.0, default=-16.9, space='buy', optimize=True)

    buy_volume_18 = DecimalParameter(1.0, 6.0, default=4.1, space='buy', optimize=True)
    buy_rsi_18 = DecimalParameter(16.0, 32.0, default=27.5, space='buy', optimize=True)
    buy_bb_offset_18 = DecimalParameter(0.98, 1.0, default=0.985, space='buy', optimize=True)

    buy_rsi_1h_min_19 = DecimalParameter(40.0, 70.0, default=63.1, space='buy', optimize=True)
    buy_chop_min_19 = DecimalParameter(20.0, 60.0, default=25.7, space='buy', optimize=True)

    buy_volume_20 = DecimalParameter(0.5, 6.0, default=2.2, space='buy', optimize=True)
    #buy_ema_rel_20 = DecimalParameter(0.97, 0.999, default=0.994, space='buy', optimize=True)
    buy_rsi_20 = DecimalParameter(20.0, 36.0, default=32.0, space='buy', optimize=True)
    buy_rsi_1h_20 = DecimalParameter(14.0, 30.0, default=23.7, space='buy', optimize=True)

    buy_volume_21 = DecimalParameter(0.5, 6.0, default=1.1, space='buy', optimize=True)
    #buy_ema_rel_21 = DecimalParameter(0.97, 0.999, default=0.998, space='buy', optimize=True)
    buy_rsi_21 = DecimalParameter(10.0, 28.0, default=11.4, space='buy', optimize=True)
    buy_rsi_1h_21 = DecimalParameter(18.0, 40.0, default=25.0, space='buy', optimize=True)
    ...
    sell_rsi_bb_1 = DecimalParameter(60.0, 80.0, default=79.4, space='sell', optimize=True)

    sell_rsi_bb_2 = DecimalParameter(72.0, 90.0, default=86.4, space='sell', optimize=True)

    sell_rsi_main_3 = DecimalParameter(77.0, 90.0, default=84.6, space='sell', optimize=True)

    sell_dual_rsi_rsi_4 = DecimalParameter(72.0, 84.0, default=77.8, space='sell', optimize=True)
    sell_dual_rsi_rsi_1h_4 = DecimalParameter(78.0, 92.0, default=83.0, space='sell', optimize=True)

    sell_ema_relative_5 = DecimalParameter(0.005, 0.05, default=0.0, space='sell', optimize=True)
    sell_rsi_diff_5 = DecimalParameter(0.0, 20.0, default=17.0, space='sell', optimize=True)

    sell_rsi_under_6 = DecimalParameter(72.0, 90.0, default=80.6, space='sell', optimize=True)

    sell_rsi_1h_7 = DecimalParameter(80.0, 95.0, default=90.6, space='sell', optimize=True)

    sell_bb_relative_8 = DecimalParameter(1.05, 1.3, default=1.059, space='sell', optimize=True)

    sell_custom_profit_0 = DecimalParameter(0.01, 0.1, default=0.055, space='sell', optimize=True)
    sell_custom_rsi_0 = DecimalParameter(30.0, 40.0, default=34.356, space='sell', optimize=True)
    sell_custom_profit_1 = DecimalParameter(0.01, 0.1, default=0.034, space='sell', optimize=True)
    sell_custom_rsi_1 = DecimalParameter(30.0, 50.0, default=48.28, space='sell', optimize=True)
    sell_custom_profit_2 = DecimalParameter(0.01, 0.1, default=0.052, space='sell', optimize=True)
    sell_custom_rsi_2 = DecimalParameter(34.0, 50.0, default=42.94, space='sell', optimize=True)
    sell_custom_profit_3 = DecimalParameter(0.06, 0.3, default=0.169, space='sell', optimize=True)
    sell_custom_rsi_3 = DecimalParameter(38.0, 55.0, default=46.41, space='sell', optimize=True)
    sell_custom_profit_4 = DecimalParameter(0.3, 0.6, default=0.554, space='sell', optimize=True)
    sell_custom_rsi_4 = DecimalParameter(40.0, 58.0, default=53.23, space='sell', optimize=True)

    sell_custom_under_profit_1 = DecimalParameter(0.01, 0.1, default=0.017, space='sell', optimize=True)
    sell_custom_under_rsi_1 = DecimalParameter(36.0, 60.0, default=51.1, space='sell', optimize=True)
    sell_custom_under_profit_2 = DecimalParameter(0.01, 0.1, default=0.071, space='sell', optimize=True)
    sell_custom_under_rsi_2 = DecimalParameter(46.0, 66.0, default=57.5, space='sell', optimize=True)
    sell_custom_under_profit_3 = DecimalParameter(0.01, 0.1, default=0.022, space='sell', optimize=True)
    sell_custom_under_rsi_3 = DecimalParameter(50.0, 68.0, default=56.4, space='sell', optimize=True)

    sell_custom_dec_profit_1 = DecimalParameter(0.01, 0.1, default=0.05, space='sell', optimize=True)
    sell_custom_dec_profit_2 = DecimalParameter(0.05, 0.2, default=0.159, space='sell', optimize=True)

    sell_trail_profit_min_1 = DecimalParameter(0.1, 0.25, default=0.212, space='sell', optimize=True)
    sell_trail_profit_max_1 = DecimalParameter(0.3, 0.5, default=0.32, space='sell', optimize=True)
    sell_trail_down_1 = DecimalParameter(0.04, 0.2, default=0.077, space='sell', optimize=True)

    sell_trail_profit_min_2 = DecimalParameter(0.01, 0.1, default=0.098, space='sell', optimize=True)
    sell_trail_profit_max_2 = DecimalParameter(0.08, 0.25, default=0.11, space='sell', optimize=True)
    sell_trail_down_2 = DecimalParameter(0.04, 0.2, default=0.144, space='sell', optimize=True)

    sell_trail_profit_min_3 = DecimalParameter(0.01, 0.1, default=0.049, space='sell', optimize=True)
    sell_trail_profit_max_3 = DecimalParameter(0.08, 0.16, default=0.12, space='sell', optimize=True)
    sell_trail_down_3 = DecimalParameter(0.01, 0.04, default=0.04, space='sell', optimize=True)

    sell_custom_profit_under_rel_1 = DecimalParameter(0.01, 0.04, default=0.0, space='sell', optimize=True)
    sell_custom_profit_under_rsi_diff_1 = DecimalParameter(0.0, 20.0, default=9.0, space='sell', optimize=True)

    sell_custom_stoploss_under_rel_1 = DecimalParameter(0.001, 0.02, default=0.0, space='sell', optimize=True)
    sell_custom_stoploss_under_rsi_diff_1 = DecimalParameter(0.0, 20.0, default=6.0, space='sell', optimize=True)
    ...
    def detect_pullback(self, df: DataFrame) -> DataFrame:
        """
        Detect market pullbacks using percent-change outlier method
        Returns DataFrame with pullback_flag column:
        1 (Outlier Up) / -1 (Outlier Down) / 0 (No pullback)
        """
        df["pb_pct_change"] = df["close"].pct_change()
        df['pb_zscore'] = qtpylib.zscore(df, window=self.pullback_window.value, col='pb_pct_change')
        df['pullback_flag'] = np.where(df['pb_zscore'] >= self.pullback_threshold.value, 1, 0)
        df['pullback_flag'] = np.where(df['pb_zscore'] <= -self.pullback_threshold.value, -1, df['pullback_flag'])
        return df
    ...
    def custom_sell(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        # Check dynamic ROI
        dynamic_roi_target = self.dynamic_roi(dataframe, trade, current_time, current_rate, current_profit, **kwargs)
        if dynamic_roi_target is not None and current_profit > dynamic_roi_target:
            return True

        return None

    def dynamic_roi(self, dataframe: DataFrame, trade: 'Trade', current_time: datetime, current_rate: float, current_profit: float, **kwargs) -> Optional[float]:
        """
        Dynamic ROI based on market conditions
        - ROI decreases over time (higher profit targets early, lower later)
        - Adjusts based on market volatility and trend
        - Considers pullback signals
        """
        if not self.roi_dynamic_enable.value:
            return None

        # Get current dataframe row
        current_candle = dataframe.iloc[-1].squeeze()
        
        # Calculate time-based ROI decay
        minutes_passed = (current_time - trade.open_date_utc).total_seconds() / 60
        hours_passed = minutes_passed / 60

        # 基础 ROI 随时间指数衰减
        initial_roi = 0.10  # 初始目标利润 10%
        min_roi = 0.02     # 最小目标利润 2%
        decay_factor = self.roi_time_weights.value  # 使用可优化的衰减因子
        
        # 指数衰减公式
        base_roi = max(
            min_roi,
            initial_roi * np.exp(-decay_factor * hours_passed)
        )

        # 市场状态分析
        # 1. 趋势强度 (ADX)
        adx = ta.ADX(dataframe, timeperiod=14).iloc[-1]
        trend_strength = min(adx / 50.0, 1.0)  # 归一化到 0-1 之间
        ...
        # 强趋势市场 - 降低 ROI 要求让利润奔跑
        if adx > self.roi_adx_threshold.value:
            market_factor *= 0.8
        
        # 高波动市场 - 提高 ROI 要求快速获利
        if volatility > self.roi_atr_multiplier.value:  # 直接使用当前 volatility 而不是比较均值
            market_factor *= 1.2
        
        # RSI 超买/超卖条件
        if rsi > 70:  # 超买
            market_factor *= 1.2  # 提高获利要求
        elif rsi < 30:  # 超卖
            market_factor *= 0.8  # 降低获利要求
        
        # Pullback 信号调整
        pullback_factor = 1.0
        if current_candle['pullback_flag'] == 1:  # 上涨回调
            pullback_factor = 1.2  # 提高获利要求
        elif current_candle['pullback_flag'] == -1:  # 下跌回调
            pullback_factor = 0.8  # 降低获利要求
        
        # 计算最终的动态 ROI
        dynamic_roi = base_roi * market_factor * pullback_factor
        
        # 确保 ROI 在合理范围内
        return max(min(dynamic_roi, initial_roi), min_roi)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for ma_type in self.ma_types:
            dataframe[f'ma_{ma_type}'] = self.ma_map[ma_type]['calculate'](dataframe, timeperiod=self.base_nb_candles_buy.value)
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        conditions.append(
            (
                ...
                self.use_pullback_protection.value &
                (dataframe['pullback_flag'] == -1) &  # Buy on downward pullback
                (dataframe['ema_50_1h'] > dataframe['ema_200_1h']) &  # Trend protection
                (dataframe['safe_dips_strict']) &
                (dataframe['volume'] > 0)
            )
        )
